...
public:
    int lengthLongestPath(string input) {
      ...
      while(getline(ss, str, '\n')) {
        int level = 0;
        // Use the '\t' to find the depth
        while(str[level++] == '\t'); 

        len = str.size() - level + 1;

        //str contains a file path
        if(str.find('.') != string::npos) {
          if(count[level-1] + len > result) {
            result = count[level-1] + len;
          }
        } else { //its dir
           ...
           // +1 for adding / after the path
           count[level] = count[level -1] + len +1;
        }
      }
      return result;  
    }
